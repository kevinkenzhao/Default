iptables -A FORWARD -s 10.20.111.0/24 -i eth1 -d 10.10.111.0/24 –j ACCEPT
...
#SSH rules
iptables -A FORWARD -p tcp -s 10.10.111.0/24 --sport 1024:65535 -i eth0 -d 10.20.111.2 --dport 22 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
iptables -A FORWARD -p tcp -s 10.20.111.2 --sport 22 -d 10.10.111.0/24 --dport 1024:65535 -i eth1 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT

#SMTP rules
iptables -A FORWARD -p tcp -s 10.20.111.2 --sport 25 -i eth1 -d 10.10.111.0/24 --dport 1024:65535 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
iptables -A FORWARD -p tcp -s 10.10.111.0/24 --sport 1024:65535 -i eth0 -d 10.20.111.2 --dport 25 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT


#3) [20 pts] The internal machine should be able to perform a nslookup of fakebook.vlab.local #GOOD RULE
iptables -A FORWARD -p udp -m udp -s 10.10.111.1 --sport 53 -i eth0 -d 10.20.111.2 --dport 1024:65535 -j ACCEPT #return traffic to internal machine from name server at 10.10.111.1; state module not required as DNS request for PTR record is made over stateless UDP
iptables -A FORWARD -p udp -m udp -s 10.20.111.2 --sport 1024:65535 -i eth1 -d 10.10.111.1 --dport 53 -j ACCEPT
...
iptables -A INPUT -p icmp --icmp-type 8 -s 10.10.111.100 -d $(/sbin/ifconfig eth0 | grep -P '(10.10.111.\d{3})' | cut -c 21-33) -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
iptables -A OUTPUT -p icmp --icmp-type 0 -s $(/sbin/ifconfig eth0 | grep -P '(10.10.111.\d{3})' | cut -c 21-33) -d 10.10.111.100 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT

iptables --policy INPUT DROP
iptables --policy OUTPUT DROP
iptables --policy FORWARD DROP

#curl https://github.com/kevinkenzhao/Default/blob/main/iptables_lab4.sh > /home/student/Desktop/iptables_lab4.sh